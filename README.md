...
1. Download the precompiled binary of the TEI Completer from [here](http://search.maven.org/remotecontent?filepath=org/humanistika/oxygen/tei-completer/1.0.1/tei-completer-1.0.1-oxygen-plugin.jar) or [build from source](#building).

2. Copy the file `tei-completer-1.0.1-oxygen-plugin.jar` to `$OXYGEN_HOME/frameworks/tei`.
...
10. In the dialog box *Add resources to the classpath*, enter the following text `${framework}/tei-completer-1.0.1-oxygen-plugin.jar`, and then click the *OK* button.
